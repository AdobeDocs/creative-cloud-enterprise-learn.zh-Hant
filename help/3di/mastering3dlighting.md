...
進入光源階段後，最好在處理材質前設定光源。 為此，您可以為整個場景指定中性灰色邊緣調色材質。 如此一來，您就能更清楚地看到及瞭解光線如何影響場景中的物件輪廓。 材質完成後，光源可能需要進一步調整。
...
您也可以擷取真實世界的相片來建立環境光線。 為此工作流程需要 360 相機 （例如 [ Ricoh Theta Z1 ](https://theta360.com/en/about/theta/z1.html) ）。 接著，相機用於包圍曝光，或拍攝相同環境的多張相片，拍攝時具有多種不同的曝光值 （從曝光不足到曝光過度）。 然後，這些相片用於建構 32 位影像，通常稱為 HDR （高動態範圍的簡短資訊）。 組合這類影像的一種方法是在Photoshop中使用「合併為 HDR」功能。 嵌入的曝亮度範圍將成為強度屬性。
...
使用定向光源可完全控制光源設定。 使用這些光源來照明場景的方式與傳統攝影相同，在傳統攝影中，每種光線都可以獨立控制，讓您建立自己的虛擬攝影照明。 最常用的光源設定之一是 3 點光源系統。
...
照明愉快！ [立即下載 Dimension 的最新版本 ] （HTTPs://creativecloud.adobe.com/apps/download/ [!DNL Dimension] ）。
