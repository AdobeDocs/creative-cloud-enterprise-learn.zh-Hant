...
![James The Abys（Mary Mastrantonio 在《Abyss》中，Mary Mastrantonio 與之接觸 CGI 水的十tata結場景](assets/Photorealistic_2.jpg)

這種將圖層化或「合成」3D 模型融入影像或視訊的技巧並不新，事實上，其原點會回到 VFX 的早期 （甚至在 1980 年代）。 令人振奮的新增功能是，此技巧已成為Adobe  [!DNL Dimension] ](https://www.adobe.com/products/dimension.html) 使用者的強大工具 [ ，也是攝影師的新工作流程。
...
[!DNL Dimension]Adobe可讓使用者使用Adobe Sensei的比對影像功能，直接在應用程式中將 2D 和 3D 元素無縫地結合。以這種方式合成元素的主要好處是，它藉由以背景影像替換完全實現的 3D 場景來加快建立逼真的影像的流程，該場景可以從現實中擷取。

![Adobe中的 [!DNL Dimension] 「符合影像」功能會分析背景影像，並評估用來擷取相機的焦點長度和位置](assets/Photorealistic_4.gif)
...
在擷取背景影像時，請注意，3D 元素會合成到背景影像中。 應有一個空白的焦點區域來保留這些物件的空間。 3D 內容通常會是最終構圖的主要焦點，因此請務必不要過分突出背景。
...
若要使用 360° 相機建立 HDR 環境，必須擷取數個 EV，才能在貼文中將 HDR 影像合併。 UV 的量並未標準化，但一般而言，您希望曝光範圍的較新端進入陰影中沒有更多資訊的點，而曝光範圍的下端會變成亮部沒有更多資訊的點。
...
對 UV 進行任何顏色調整後，可以暫時將其匯出到個別檔案，然後合併到 Photoshop。 檔案類型應視來源而定，但在任何一種情況下都不要使用像 JPEG 這樣的壓縮格式。 在Photoshop使用檔案>「合併為 HDR Pro」>自動執行...然後選取所有匯出的 UV。
...
最後步驟是移除影像最底端顯示的任何陰影和三腳架腿，並調整影像的預設曝亮度，以正確地照亮場景。 您可以使用「Photoshop」中的仿製工具移除細節。 調整曝亮度應與背景 [!DNL Dimension] 一起進行，因為 HDR IBL 的曝亮度值是 3D 物件的光源值。
...
場景完成後，使用者的工作流程就相當簡單。 只需將您自己的模型或任何 [ Adobe  [!DNL Stock]  3D ](https://stock.adobe.com/3d-assets) 內容直接拖放到影像中，就如同拍攝相片時所處的內容一樣呈現。 這會開啟建立高度逼真的廣告內容的新途徑，或是能夠在許多不同的情境中反覆運算設計。

![合成廚房計數器頂端場景的 3D 設備逼真虛擬照片](assets/Photorealistic_25.png)

最終成果是現實和 3D 的具說服力的融合，可協助一般使用者以最少的精力來建立逼真的影像。 利用我們建立來展示工作流程的 ](https://assets.adobe.com/public/3926726a-2a17-43d4-4937-6d84a4d29338) 自由  [!DNL Dimension]  授權，自己動手嘗試 [ 。
